...
def test_compute_coverage_full_match():
    snapshot = [{"id": 1, "writing": "你好", "reading": "nǐhǎo", "definition_en": "hello"}]
    result = GenerationResult(title="标题", body="你好，世界。", usage=UsageMetadata(1, 1, 2))
...
def test_run_single_success_records_coverage_and_latency():
    snapshot = [{"id": 1, "writing": "菜单", "reading": "càidān", "definition_en": "menu"}]
    fixture = _fixture(snapshot)
    provider = FakeStoryGenerationProvider(scenario="success")
    spec = ProviderSpec(label="fake", provider=provider, model="fake-model", base_url="https://fake.test/v1/chat/completions")
    ...
    assert outcome.coverage == 1.0  # FakeStoryGenerationProvider's default body contains 菜单
...
def test_run_comparison_covers_full_matrix():
    fixtures = [
        _fixture([{"id": 1, "writing": "菜单", "reading": "càidān", "definition_en": "menu"}], name="f1"),
        _fixture([{"id": 2, "writing": "水", "reading": "shuǐ", "definition_en": "water"}], name="f2"),
    ]
    specs = [
        ProviderSpec(label="fake-a", provider=FakeStoryGenerationProvider(scenario="success"), model="a", base_url="https://fake-a.test"),
        ProviderSpec(label="fake-b", provider=FakeStoryGenerationProvider(scenario="success"), model="b", base_url="https://fake-b.test"),
    ]
    ...
    assert pairs == {("f1", "fake-a"), ("f1", "fake-b"), ("f2", "fake-a"), ("f2", "fake-b")}


def test_eval_fixtures_cover_required_categories():
    categories = {f.category for f in EVAL_FIXTURES}
    assert {"hsk_level", "ambiguous_words", "short_list", "mixed_known"}.issubset(categories)
...
def test_to_markdown_renders_supplied_manual_quality_notes():
    ...
    fixture = _fixture(snapshot)
    provider = FakeStoryGenerationProvider(scenario="success")
    spec = ProviderSpec(label="fake", provider=provider, model="fake-model", base_url="https://fake.test")
    outcome = run_single(fixture, spec)
...
def test_to_markdown_escapes_pipes_and_newlines_in_notes_table_cell():
    ...
    table_lines = [line for line in summary_section.splitlines() if line.startswith("|")]
